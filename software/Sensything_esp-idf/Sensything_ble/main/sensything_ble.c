...
uint8_t qwiic_service_uuid[16]= {0xd0,0x36,0xba,0x8c,0xda,0xd1,0x4c,0xae,0xb8,0x7d,0x48,0x44,0x00,0x11,0x5c,0xcd};     //custom 128bit service UUID for RR
uint8_t qwiic_char_uuid[16]= {0xd0,0x36,0xba,0x8c,0xda,0xd1,0x4c,0xae,0xb8,0x7d,0x48,0x44,0x01,0x11,0x5c,0xcd};    
...
static void gatts_profile_qwiic_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param);

uint8_t beat_flag = 0;
uint8_t * ads1220data;
uint8_t value_ads1220[16];

uint16_t attr_handle_hr = 0x002a;
uint16_t attr_handle_rr = 0x002a;
uint16_t attr_handle_qwiic = 0x002a;
uint16_t attr_handle_bat = 0x002a;

uint16_t conn_id_indicate_hr=0;
uint16_t conn_id_indicate_rr=0;
uint16_t conn_id_indicate_qwiic=0;
...
esp_gatt_if_t gatts_if_for_qwiic = ESP_GATT_IF_NONE;
...
static struct gatts_profile_inst gl_profile_tab[PROFILE_NUM] = 
{
    ...
    [PROFILE_B_APP_ID] = 
    {
        .gatts_cb = gatts_profile_qwiic_event_handler,                  
        .gatts_if = ESP_GATT_IF_NONE,      
    },
...
static void gatts_profile_bat_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_CREATE_EVT:
        ...
        gl_profile_tab[PROFILE_A_APP_ID].char_uuid.uuid.uuid16 = GATTS_CHAR_UUID_BAT;
...
static void gatts_profile_qwiic_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_REG_EVT:       
        ...
        gl_profile_tab[PROFILE_B_APP_ID].service_id.id.uuid.len = ESP_UUID_LEN_128;
            ...
            gl_profile_tab[PROFILE_B_APP_ID].service_id.id.uuid.uuid.uuid128[i] = qwiic_service_uuid[i];
        }
        
        esp_ble_gatts_create_service(gatts_if, &gl_profile_tab[PROFILE_B_APP_ID].service_id, GATTS_NUM_HANDLE_QWIIC);
        break;
    case ESP_GATTS_READ_EVT: {
        esp_gatt_rsp_t rsp;
        memset(&rsp, 0, sizeof(esp_gatt_rsp_t));
    ...
    case ESP_GATTS_CREATE_EVT:
        ...
        for(int i=0; i<16; i++){
                ...
                gl_profile_tab[PROFILE_C_APP_ID].char_uuid.uuid.uuid128[i]=qwiic_char_uuid[i];
        }
    ...
    case ESP_GATTS_ADD_CHAR_EVT:      
        ...
        attr_handle_qwiic = param->add_char.attr_handle;
        esp_ble_gatts_add_char_descr(gl_profile_tab[PROFILE_C_APP_ID].service_handle, &gl_profile_tab[PROFILE_C_APP_ID].descr_uuid,
                                     ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE, NULL, NULL);
        break;           
    case ESP_GATTS_CONNECT_EVT:
        gl_profile_tab[PROFILE_C_APP_ID].conn_id = param->connect.conn_id;      
        gatts_if_for_qwiic = gatts_if;         
        conn_id_indicate_qwiic = param->connect.conn_id;   
        
        break;
    case ESP_GATTS_DISCONNECT_EVT:
            gatts_if_for_qwiic = ESP_GATT_IF_NONE;
        break;
...
void update_bat(uint8_t value)
{
    ...
    if(gatts_if_for_hr != ESP_GATT_IF_NONE){
        esp_ble_gatts_send_indicate(gatts_if_for_hr, conn_id_indicate_hr, attr_handle_hr, 1, &(ble_data_update.bat), false);
    }
}
...
static void notify_task(void* arg) {
	...
	while (true) {
        ...
        if(bme280_found){
            ...
            esp_ble_gatts_send_indicate(gatts_if_for_qwiic, conn_id_indicate_qwiic, attr_handle_qwiic, BME280_DATABUFF_LEN, get_bme280_data(), false);
        }
        vTaskDelay(1000/ portTICK_PERIOD_MS);
